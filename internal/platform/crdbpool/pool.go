...
//
// The failure it produces is already handled. CockroachDB raises SQLSTATE 57014 (query_canceled),
// which errclass classifies Transient through its class-57 prefix rule, so a timed-out statement
// re-enters the consumer's existing retry ladder instead of dead-lettering valid work. That
// property is what makes this safe to turn on, and errclass_test.go pins it.
package crdbpool

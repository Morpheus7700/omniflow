// Package delivery bounds the call that finishes a Kafka message's handling: the offset commit.
//
// Duplicated from internal/platform/delivery rather than imported — viz-gateway is a separate Go
// module and cannot reach the root module's internal/, as with health and crdbpool. This service
// has no DLQ (it is a read model; a message it cannot project is dropped, not dead-lettered), so
// only the commit half applies here.
//

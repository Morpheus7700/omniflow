...
// NOTE ON DUPLICATION: viz-gateway is its own Go module (module omniflow/services/viz-gateway) and
// therefore cannot import omniflow/internal/platform/health — the same boundary that forces it to
// keep its own copy of any shared helper. This file is a verbatim copy of that package. If you
// change one, change both; the tests in each module are what catch the drift.
package health
